...
DEMO_DIR=/home/brochwer/repos/brochwer/beyond-tutorial-k8s-old

source $SCRIPT_DIR/vars.sh
source $SCRIPT_DIR/common.sh

if [[ -z $REGISTRY || -z $USERID || -z $APPNAME ]]
then
    echo "ERROR: Mandatory enviroment varibles (REGISTRY, USERID & APPNAME) not set ... Aborting"
...
then 
    DEMO3_COMMIT=$(git log --oneline main | grep "DEMO 3" | awk '{print $1}')
    set -x 
...
then
    set -x
    podman build -t "$REGISTRY/$USERID/$APPNAME-psql" $DEMO_DIR
    podman push "$REGISTRY/$USERID/$APPNAME-psql"
    set +x
fi
